...
class MonolithicEncoder(BaseEncoder):
    def condition_sat_in_abstract_step(self, action, it, c, w, h):
        ...
        if it.is_left_open():
            start_condition_after_last_concrete_step = self.mgr.LE(
                last_concrete_step_time, smt_tp_1
            )
        else:
    ...
    def encode_durative_action(self, action: DurativeAction, i: int, h: int):
        ...
        terms.append(self.encode_action_duration(action, i))
        if not self.optimal:
            terms.append(
                self.mgr.LE(
                    self.mgr.Plus(self.t(i), self.dur(action, i)), self.t(h - 1)
                )
            )
        ...
        for t, le in action.effects.items():
            if t.is_from_start() and t.delay == 0:
                ...
                if self.optimal:
                    ...
                    effect_formula = self.mgr.Or(
                        effect_formula, effect_in_abstract_step
                    )
                terms.append(effect_formula)
            else:
                l2 = []
                for k in range(i, h):
    ...
    def encode_timed_goals(self, h: int, optimal: bool):
        ...
        for it, lg in self.problem.timed_goals.items():
            ...
            if not optimal:
                res.append(
                    self.mgr.LE(self.encode_problem_tp(it.upper, h), self.t(h - 1))
                )
        ...
        for action, timing, eff in fluent_touchers_gen:
            if action is None:
                ...
                for i in range(1, h):
                    ...
                    effect_in_abstract_step = self.mgr.GT(
                        effect_time, last_concrete_step_time
                    )
    ...
    def encode_step(self, h: int) -> Tuple[Any, Any]:
        ...
        for fluent_mod_var, (fluent, fluent_exp) in self.fluent_mod_var.items():
            res.append(
                self.mgr.Iff(
                    fluent_mod_var,
                    self.encode_fluent_mod_formula(fluent, fluent_exp, h),
                )
            )

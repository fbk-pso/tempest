...
Event = tuple[Action | None, Timing, frozenset[FNode], frozenset[Effect]]
...
Toucher = tuple[Action | None, Timing | None, Effect]
...
class BaseEncoder(ABC):
    def __init__(
        ...
        if ground_abstract_step and optimal:
            try:
                with self.problem.environment.factory.Compiler(
                    name=grounder_name,
        ...
        self.abstract_step_actions = (
            grounded_problem.actions if self.ground_abstract_step else self.actions
        )
        self.abstract_step_metrics = grounded_problem.quality_metrics
        self.fluent_mod_var: dict[Any, Any] = {}

        self.converters: dict[Any, Any] = {}
        ...
        self.touchers: dict[Fluent, dict[FNode, list[Toucher]]] = self._get_touchers(
            problem
        )
        self.abstract_step_touchers: dict[Fluent, dict[FNode, list[Toucher]]] = (
            self.touchers
        )
    ...
    def converter(self, i: int, w: int | None, scope: Action | None) -> SMTConverter:
    ...
    def t_a(self, a: Action, h: int | None) -> SMTFNode:
        return self.symenc.t_a(a, h)

    def t_last(self) -> SMTFNode:
        return self.symenc.t_last()

    def a(self, action: Action, i: int) -> SMTFNode:
        return self.symenc.action(action, i)

    def param(self, action: Action, parameter: Parameter, i: int) -> SMTFNode:
        return self.symenc.parameter(action, parameter, i)

    def dur(self, action: Action, i: int) -> SMTFNode:
    ...
    def encode_tp(
        ...
        smt_t = (
            self.t(i)
            if (h is None and not abstract) or (h is not None and i < h)
            else self.t_a(action, h)
        )
    ...
    def encode_problem_tp(self, t: Timing, h: int | None) -> SMTFNode:
        if t.is_from_start():
    ...
    def encode_effect(
        self, action: Action | None, e: Effect, i: int, w: int
    ) -> SMTFNode:
    ...
    def encode_condition_or_goal(
        ...
        if self._is_always_empty_interval(it):
            ...
            smt_tp = smt_tp_1
            cond = self.mgr.And(
                self.mgr.LT(self.t(i - 1), smt_tp), self.mgr.GE(self.t(i), smt_tp)
            )
            formula = self.mgr.Implies(cond, self.to_smt(c, i - 1, w, scope=action))
        elif it.is_left_open():  # open and left open are the same
            cond_1 = self.mgr.And(
                self.mgr.LE(self.t(i - 1), smt_tp_1),
                self.mgr.GT(self.t(i), smt_tp_1),
                non_empty_interval_operand(smt_tp_2, smt_tp_1),
            )
            formula_1 = self.mgr.Implies(cond_1, self.to_smt(c, i - 1, w, scope=action))
            cond_2 = self.mgr.And(
                self.mgr.LT(self.t(i), smt_tp_2), self.mgr.GT(self.t(i), smt_tp_1)
            )
            ...
            cond_1 = self.mgr.And(
                self.mgr.LT(self.t(i - 1), smt_tp_1),
                self.mgr.GE(self.t(i), smt_tp_1),
                non_empty_interval_operand(smt_tp_2, smt_tp_1),
            )
            formula_1 = self.mgr.Implies(cond_1, self.to_smt(c, i - 1, w, scope=action))
            cond_2 = self.mgr.And(
                self.mgr.LT(self.t(i), smt_tp_2), self.mgr.GE(self.t(i), smt_tp_1)
            )
            formula_2 = self.mgr.Implies(cond_2, self.to_smt(c, i, w, scope=action))
            formula = self.mgr.And(formula_1, formula_2)
    ...
    def encode_action_duration(self, action: DurativeAction, i: int) -> SMTFNode:
        smt_dur = self.dur(action, i)
        ...
        if duration.is_left_open():
            terms.append(self.mgr.GT(smt_dur, smt_lower))
        else:
            terms.append(self.mgr.GE(smt_dur, smt_lower))
        if duration.is_right_open():
            terms.append(self.mgr.LT(smt_dur, smt_upper))
        else:
            terms.append(self.mgr.LE(smt_dur, smt_upper))
        return self.mgr.And(terms)

    def encode_instantaneous_action(
        self, action: InstantaneousAction, i: int
    ) -> SMTFNode:
    ...
    def encode_frame_axiom(self, i: int, h: int | None) -> SMTFNode:
        res = []
    ...
    def is_mutex(
        self,
        a_precond: Iterable[FNode],
        a_effects: Iterable[Effect],
        b_precond: Iterable[FNode],
        b_effects: Iterable[Effect],
    ) -> list[tuple[FNode, FNode]]:
        a_p: dict[Any, list] = {}
        for p in a_precond:
        ...
        def add_red_fluents(p: dict[Any, list], effect: Effect) -> None:
            for x in chain(
                *map(
                    self._get_sorted_free_vars,
                    (effect.condition, effect.fluent, effect.value),
                )
            ):
    ...
    def encode_non_self_overlapping(self, i: int) -> SMTFNode:
        res = []
        for act in self.actions:
            ...
            for j in range(1, i):
                ...
                res.append(
                    self.mgr.Implies(
                        actions_and_same_parameters, end_a_j_before_start_a_i
                    )
                )
        return self.mgr.And(res)

    def initial_state(self) -> SMTFNode:
        res = []
    ...
    def extract_plan(
        self, model: pysmt.solvers.solver.Model, h: int
    ) -> SequentialPlan | TimeTriggeredPlan:
        res = []
        for i in range(1, h):
    ...
    def _get_events(self) -> set[Event]:
            ...
            if isinstance(act, InstantaneousAction):
                events.add(
                    (
                        act,
                        start_timing,
                        frozenset(act.preconditions),
                        frozenset(act.effects),
                    )
                )
            else:
        ...
        events.add(
            (None, global_end_timing, frozenset(self.problem.goals), frozenset())
        )

        return events

    def _get_mutex_couples(self) -> list[tuple]:
        mutex_couples: list[tuple] = []
        ...
        for i, event_a in enumerate(events):
            ...
            for event_b in events[i + 1 :]:
                ...
                if mutex_conds:
                    mutex_couples.append(
                        ((action_a, timing_a), (action_b, timing_b), mutex_conds)
                    )
    ...
    def _get_touchers(
        self, problem: Problem
    ) -> dict[Fluent, dict[FNode, list[Toucher]]]:
        touchers: dict[Fluent, dict[FNode, list[Toucher]]] = {}
    ...
    def encode_mutex_constraints(self, i: int, j: int, h: int | None) -> SMTFNode:
        res = []

        def encode_timing(action: Action | None, timing: Timing, step: int) -> SMTFNode:
        ...
        for mutex_element in self._mutex_couples:
            ...
            def is_global_end(timing: Timing) -> bool:
                return timing.is_global() and timing.is_from_end()

            if (
                (i == j and action_a == action_b)
                or is_global_end(timing_a)
                or is_global_end(timing_b)
                or (i != j and timing_a == timing_b)
            ):
                continue
            ...
            for f1, f2 in mutex_conds:
                mutex_cond_list.append(
                    self.mgr.And(
                        [
                            self.mgr.EqualsOrIff(
            ...
            if action_a is None:
                ...
                res.append(
                    self.mgr.Not(
                        self.mgr.And(b_j, same_timing, self.mgr.Or(mutex_cond_list))
                    )
                )
            else:
                ...
                if action_b is None:
                    ...
                    res.append(
                        self.mgr.Not(
                            self.mgr.And(a_i, same_timing, self.mgr.Or(mutex_cond_list))
                        )
                    )
                else:
                    # both are actions
                    ...
                    res.append(
                        self.mgr.Not(
                            self.mgr.And(
                                a_i, b_j, same_timing, self.mgr.Or(mutex_cond_list)
                            )
                        )
                    )
    ...
    def encode_increasing_time(self, i: int) -> SMTFNode:
        if i == 1:
        ...
        assert self.epsilon > 0
        return self.mgr.LE(
            self.mgr.Plus(self.t(i - 1), self.mgr.Real(self.epsilon)), self.t(i)
        )

    @cache
    def _fluent_mod(self, fluent: Fluent, fluent_exp: FNode | None) -> SMTFNode:
    ...
    @cache
    def fluent_mod(
        self, fluent_exp: FNode, a: Action | None, w: int | None
    ) -> SMTFNode:
        ...
        for parameters_value in self._get_possible_parameters_assignments(
            ...
            for param_exp, obj_exp in assignments.items():
                sub_res.append(
                    self.mgr.EqualsOrIff(
                        self.to_smt(param_exp, w, w, scope=a), self.to_smt(obj_exp, w)
                    )
                )
    ...
    @cache
    def _get_possible_parameters_assignments(
        self, parameters: tuple[FNode, ...]
    ) -> tuple[tuple[FNode, ...], ...]:
    ...
    def encode_abstract_instantaneous_action(
        self, action: InstantaneousAction, h: int
    ) -> SMTFNode:
        assert self.optimal
        terms = []
        a_h = self.a(action, h)
        ...
        for p in action.preconditions:
            ...
            condition_abstract_step = self.mgr.Or(
                self.fluent_mod(exp, action, h) for exp in self._get_sorted_free_vars(p)
            )
            terms.append(
                self.mgr.Or(condition_last_concrete_step, condition_abstract_step)
            )
        return self.mgr.Implies(a_h, self.mgr.And(terms))

    def phi_sched(self, h: int) -> SMTFNode:
        ...
        for interval, goals in self.problem.timed_goals.items():
            ...
            interval_in_abstract_step = self.mgr.GT(
            ...
            interval_not_empty = empty_interval_operand(
                self.encode_problem_tp(interval.lower, h),
                self.encode_problem_tp(interval.upper, h),
            )
            for goal in goals:
                ...
                res.append(
                    self.mgr.Implies(
                        self.mgr.And(
                            goal_not_satisfied,
                            interval_in_abstract_step,
                            interval_not_empty,
                        ),
                        self._phi_sched_parametrized_formula(
                            goal, timing, interval.is_left_open(), None, None, h
                        ),
                    )
                )
            ...
            for s in range(1, h):
                ...
                for interval, cl in a.conditions.items():
                    ...
                    if interval.is_left_open():
                        interval_in_abstract_step = self.mgr.GE(
                            self.encode_tp(a, timing, s, h), self.t(h - 1)
                        )
                    else:
                        interval_in_abstract_step = self.mgr.GT(
                            self.encode_tp(a, timing, s, h), self.t(h - 1)
                    ...
                    if interval.is_left_open() or interval.is_right_open():
                        empty_interval_operand = self.mgr.LT
                    interval_not_empty = empty_interval_operand(
                        self.encode_tp(a, interval.lower, s, h),
                        self.encode_tp(a, interval.upper, s, h),
                    )
                    for cond in cl:
                        cond_not_satisfied = self.mgr.Not(
                            self.to_smt(cond, h - 1, s, scope=a)
                        )
                        sub_res.append(
                            self.mgr.Implies(
                                self.mgr.And(
                                ...
                                self._phi_sched_parametrized_formula(
                                    cond, timing, interval.is_left_open(), a, s, h
                                ),
                            )
                        )
        ...
        for a in self.abstract_step_actions:
            assert self.epsilon is not None and self.epsilon > 0
            sub_res = []
            sub_res.append(
                self.mgr.LE(
                    self.mgr.Plus(self.t(h - 1), self.mgr.Real(self.epsilon)),
                    self.t_a(a, h),
                )
            )
            if isinstance(a, InstantaneousAction):
                ...
                for interval, cl in a.conditions.items():
                    ...
                    if interval.is_left_open() or interval.is_right_open():
                        empty_interval_operand = self.mgr.LT
                    interval_not_empty = empty_interval_operand(
                        self.encode_tp(a, interval.lower, h, h),
                        self.encode_tp(a, interval.upper, h, h),
                    )
                    for cond in cl:
                        cond_not_satisfied = self.mgr.Not(
                            self.to_smt(cond, h - 1, h, a)
                        )
                        sub_res.append(
                            self.mgr.Implies(
                                self.mgr.And(cond_not_satisfied, interval_not_empty),
                                self._phi_sched_parametrized_formula(
                                    cond, timing, interval.is_left_open(), a, h, h
                                ),
                            )
                        )
    ...
    def encode_density_constraint(self, i: int) -> SMTFNode:
        t_i = self.t(i)
    ...
    def uses_abstact_step(self, i: int, h: int | None) -> SMTFNode:
    ...
    def _phi_sched_parametrized_formula(
        self,
        phi: FNode,
        t: Timing | None,
        left_open: bool,
        ...
        for fluent_exp in self._get_sorted_free_vars(phi):
            ...
            if not fluent_params or not self.ground_abstract_step:
                res.append(
                    self.mgr.And(
                        self.fluent_mod(fluent_exp, a, w),
                        self._phi_sched_formula(phi_time, fluent_exp, left_open, h),
                    )
                )
            else:
                assert w is not None and w < h, (
                    "parameters should come only from actions started in concrete steps"
                )
                for parameter_assignment in self._get_possible_parameters_assignments(
                    ...
                    for param_exp, obj_exp in assignments.items():
                        p_eq.append(
                            self.mgr.EqualsOrIff(
                                self.to_smt(param_exp, w, w, scope=a),
                                self.to_smt(obj_exp, w),
                            )
                        )
                    ...
                    res.append(
                        self.mgr.And(
                            parameters_equality,
                            self.fluent_mod(ground_fluent_exp, a, w),
                            self._phi_sched_formula(
                                phi_time, ground_fluent_exp, left_open, h
                            ),
                        )
                    )
    ...
    def _phi_sched_formula(
        self, phi_time: SMTFNode, fluent_exp: FNode, left_open: bool, h: int
    ) -> SMTFNode:
        res = []
        last_t = self.t(h - 1)
        if self.ground_abstract_step:
            abstract_touchers_dict = self.abstract_step_touchers.get(
                fluent_exp.fluent(), {}
            )
            abstract_touchers: Iterable[Toucher] = abstract_touchers_dict.get(
                fluent_exp, []
            )
        else:
            abstract_touchers_dict = self.abstract_step_touchers.get(
        ...
        for b, t, _e in abstract_touchers:
                ...
                for k in range(1, h):
                    ...
                    if self.ground_abstract_step:
                        ...
                        for param_exp, obj_exp in parameters_assignment.items():
                            parameters_equality.append(
                                self.mgr.EqualsOrIff(
                                    self.to_smt(param_exp, k, k, scope=lifted_a),
                                    self.to_smt(obj_exp, k),
                                )
                            )
                    ...
                    b_k_formula.append(
                        self.mgr.LE(
                            self.mgr.Plus(last_t, self.mgr.Real(self.epsilon)), e_k_time
                        )
                    )
                    if left_open:
                        ...
                        b_k_formula.append(
                            self.mgr.LE(
                                self.mgr.Plus(e_k_time, self.mgr.Real(self.epsilon)),
                                phi_time,
                            )
                        )
            ...
            h_step_formula.append(
                self.mgr.LE(
                    self.mgr.Plus(last_t, self.mgr.Real(self.epsilon)), e_h_time
                )
            )
            if left_open:
                ...
                h_step_formula.append(
                    self.mgr.LE(
                        self.mgr.Plus(e_h_time, self.mgr.Real(self.epsilon)), phi_time
                    )
                )
    ...
    def _get_sorted_parameters(
        self, exp: FNode, action: Action | None
    ) -> tuple[FNode, ...]:
    ...
    def objective_to_minimize(
        self, i: int, h: int | None
    ) -> tuple[list[Any], list[SMTFNode] | None]:
        assert len(self.problem.quality_metrics) < 2, (
            "Problem has more than one quality metric"
        )
        metric = (
            self.problem.quality_metrics[0] if self.problem.quality_metrics else None
        )
        if metric is None or metric.is_minimize_makespan():
            ...
            for a in self.abstract_step_actions:
                ...
                if isinstance(a, InstantaneousAction):
                    ...
                    terms.append(
                        self.mgr.GE(makespan, self.mgr.Plus(t_a, self.dur(a, i + 1)))
                    )
            if self.secondary_objective is None:
                ...
                return [
                    MinimizationGoal(
                        self.mgr.Ite(
                            self.uses_abstact_step(i, h),
                            self.mgr.Plus(
                                makespan,
                                self.mgr.Real((1, 1000))
                                if self.epsilon is None
                                else self.mgr.Real(self.epsilon / 2),
                            ),
                            makespan,
                        )
                    )
            ...
            if self.secondary_objective is None:
                ...
                max_smt_goal.add_soft_clause(
                    self.mgr.Not(self.uses_abstact_step(i, h)), 0.001
                )
                objs = [max_smt_goal]
            elif self.secondary_objective == "lexicographic":
                objs = [
    ...
    def _get_max_smt_and_minimization_goal(
        self, metric: MinimizeActionCosts, h: int
    ) -> tuple[MaxSMTGoal, MinimizationGoal]:
            ...
            for assignments, cost in self._get_action_costs(metric, a):
                ...
                for i in range(range_lim):
                    ...
                    for param_exp, obj_exp in assignments.items():
                        ...
                        clauses.append(
                            self.mgr.EqualsOrIff(
                                self.to_smt(param_exp, i, scope=a),
                                self.to_smt(obj_exp, i),
                            )
                        )
                    costs.append(
                        self.mgr.Ite(
                            self.mgr.And(clauses),
                            self.mgr.Real(weight),
                            self.mgr.Real(0),
                        )
                    )
                    max_smt_goal.add_soft_clause(
                        self.mgr.Not(self.mgr.And(clauses)), weight
                    )
    ...
    def _get_action_costs(
        self, metric: MinimizeActionCosts, action: Action
    ) -> list[tuple[dict[FNode, FNode], FNode]]:

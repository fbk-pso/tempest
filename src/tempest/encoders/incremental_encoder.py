...
class IncrementalEncoder(BaseEncoder):
    ...
    def encode_incremental_durative_action(
        ...
        if not self.optimal:
            conjunction.append(
                self.mgr.LE(
                    self.mgr.Plus(self.t(i), self.dur(action, i)), self.t_last()
                )
            )
                ...
                if self.optimal:
                    ...
                    if it.is_left_open():
                        start_condition_after_last_concrete_step = self.mgr.LE(
                            self.t_last(), smt_tp_1
                        )
                    else:
                        start_condition_after_last_concrete_step = self.mgr.LT(
                            self.t_last(), smt_tp_1
                        )
                    ...
                    temp_l.append(
                        self.mgr.Implies(
                            ...
                            self.mgr.Implies(
                                ...
                                self.mgr.Or(
                                    condition_last_concrete_step,
                                    condition_abstract_step,
                                ),
                            ),
                        )
                    )
        ...
        terms.append(self.mgr.Implies(self.a(action, i), self.mgr.And(conjunction)))
    ...
    def encode_abstract_durative_action(
        ...
        temp_l.append(
            self.encode_action_duration(action, i)
        )  # TOFIX if non-static fluents in duration
        ...
        for it, lc in action.conditions.items():
            ...
            temp_l.append(
                self.mgr.Implies(
                    self.mgr.GT(
                        self.encode_tp(action, it.lower, i, None, True), self.t_last()
                    ),
    ...
    def encode_fluent_mod_formula_step_zero(
        ...
        if self.ground_abstract_step:
            ...
            fluent_touchers_gen: Iterable[Any] = abstract_fluent_touchers.get(
                fluent_exp, []
            )
        else:
        ...
        for action, timing, eff in fluent_touchers_gen:
            if action is None:
                til_in_abstract_step = self.mgr.GT(
                    self.encode_problem_tp(timing, None), self.t_last()
                )
    ...
    def encode_fluent_mod_formula(
        ...
        for action, timing, eff in fluent_touchers_gen:
            ...
            if timing is None:
                res.append(
                    self.mgr.Implies(
                        self.chain_var(action, ev, i, 0),
                        self.chain_var(action, ev, i + 1, 0),
                    )
                )
                temp_res.append(
                    self.mgr.Implies(
                        self.chain_var(action, ev, i + 1, 0), self.a(action, i + 1)
                    )
                )
            else:
                ...
                for param_exp, obj_exp in parameters_assignment.items():
                    parameters_equality.append(
                        self.mgr.EqualsOrIff(
                            self.to_smt(
                                self.em.ParameterExp(param_exp),
                                i,
                                i,
                                scope=concrete_action,
                            ),
                ...
                effect_performed_in_abstract_step = self.mgr.And(
                    a_i, p_eq, effect_in_abstract_step
                )
                res.append(
                    self.mgr.Implies(
                        self.chain_var(action, ev, i, 0),
                        self.mgr.Or(
                            effect_performed_in_abstract_step,
                            self.chain_var(action, ev, i + 1, 0),
                        ),
                    )
                )
    ...
    def encode_step_zero(self) -> Any | None:
        ...
        if self.ground_abstract_step:
            for fluent in self.problem.fluents:
                for fluent_exp in get_all_fluent_exp(self.problem, fluent):
                    ...
                    res.append(
                        self.mgr.Iff(
                            mod_f,
                            self.encode_fluent_mod_formula_step_zero(
                                fluent, fluent_exp
                            ),
                        )
                    )
            ...
            for fluent in self.problem.fluents:
                ...
                res.append(
                    self.mgr.Iff(
                        mod_f, self.encode_fluent_mod_formula_step_zero(fluent, None)
                    )
                )
    ...
    def encode_step(self, i: int) -> tuple[Any, Any]:
        ...
        for t, le in self.problem.timed_effects.items():
            ...
            if self.optimal:
                ...
                temp_res.append(
                    self.mgr.Implies(
                        self.chain_var(None, ev, i + 1, 0),
                        self.mgr.GT(smt_tp, self.t_last()),
                    )
                )
            else:
                temp_res.append(
            ...
            if it.lower == it.upper and it.lower.is_from_end() and it.lower.delay == 0:
                ...
                if self.optimal:
                    ...
                    temp_res.append(
                        self.mgr.Implies(
                            self.mgr.LT(
                                self.t(i), self.encode_problem_tp(it.lower, None)
                            ),
                            self.mgr.Or(mod_f, self.to_smt(g, i, 0)),
                        )
                    )
                else:
                    res.append(
                        self.mgr.LE(
                            self.encode_problem_tp(it.upper, None), self.t_last()
                        )
                    )

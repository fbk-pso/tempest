...
class ProblemEncoder:
    def __init__(self, problem, pysmt_env=None):
        ...
        self._fluents_modification_map: Dict[Fluent, Dict[FNode, Set[Tuple[Optional[Action], Timing, bool, Optional[int]]]]] = self._create_fluents_modification_map()
    ...
    def encode_mutex_constraints(self, i, h):
        ...
        for fluent, modifications_map in self._fluents_modification_map.items():
                ...
                for action_a, timing_a, is_write_a, til_id_a in modifications_set_a:
                    for fluent_exp_b, modifications_set_b in modifications_map.items():
                        for action_b, timing_b, is_write_b, til_id_b in modifications_set_b:
                            for j in range(1, h):
                                if (not is_write_a and not is_write_b) or (til_id_a is not None and til_id_a == til_id_b) or (i == j and action_a == action_b):
                                    continue
    ...
    def monolithic_bounded_planning(self, h):
        ...
        for i in range(1, h):
            ...
            if old:
                ...
                res.append(self.encode_mutex_constraints(i, h))

...
class TestConverter(TestCase):
    def test_converter(self):
        ...
        counter_param = problem.add_fluent(
            "counter_param", IntType(), default_initial_value=0, param=CounterType
        )
        static = problem.add_fluent("static", IntType(), default_initial_value=5)
        static_param = problem.add_fluent(
            "static_param", IntType(), default_initial_value=1, param=CounterType
        )

        # real fluents
        real_counter = problem.add_fluent(
            "real_counter", RealType(), default_initial_value=0
        )
        real_counter_param = problem.add_fluent(
            "real_counter_param", RealType(), default_initial_value=0, param=CounterType
        )
        real_static = problem.add_fluent(
            "real_static", RealType(), default_initial_value=5
        )
        real_static_param = problem.add_fluent(
            "real_static_param", RealType(), default_initial_value=1, param=CounterType
        )

        # bool fluents
        bool_counter = problem.add_fluent(
            "bool_counter", BoolType(), default_initial_value=False
        )
        bool_counter_param = problem.add_fluent(
            "bool_counter_param",
            BoolType(),
            default_initial_value=False,
            param=CounterType,
        )
        bool_static = problem.add_fluent(
            "bool_static", BoolType(), default_initial_value=False
        )
        bool_static_param = problem.add_fluent(
            "bool_static_param",
            BoolType(),
            default_initial_value=False,
            param=CounterType,
        ...
        increase_counter_param = DurativeAction(
            "increase counter param", counter_obj=CounterType
        )
        counter_obj_param = increase_counter_param.counter_obj
        increase_counter_param.set_fixed_duration(1)
        increase_counter_param.add_increase_effect(
            EndTiming(), counter_param(counter_obj_param), 1
        )
        increase_counter.add_increase_effect(
            EndTiming(), real_counter_param(counter_obj_param), 1
        )
        increase_counter.add_effect(
            EndTiming(),
            bool_counter_param(counter_obj_param),
            Not(bool_counter_param(counter_obj_param)),
        )
        ...
        expressions_oracle_scope = [
            ...
            (
                counter_param(counter_obj_param),
                f"(('parameter_{increase_counter_param.name}_{counter_obj_param.name}@{{w}}' = 2.0) ? fluent_{counter_param.name}_{c3.name}@{{i}} : (('parameter_{increase_counter_param.name}_{counter_obj_param.name}@{{w}}' = 1.0) ? fluent_{counter_param.name}_{c2.name}@{{i}} : fluent_{counter_param.name}_{c1.name}@{{i}}))",
                increase_counter_param,
            ),
            ...
            (
                counter + counter_param(c1),
                f"(fluent_{counter.name}@{{i}} + fluent_{counter_param.name}_{c1.name}@{{i}})",
                None,
            ),
            (
                counter / counter_param(counter_obj_param),
                f"(fluent_{counter.name}@{{i}} / (('parameter_{increase_counter_param.name}_{counter_obj_param.name}@{{w}}' = 2.0) ? fluent_{counter_param.name}_{c3.name}@{{i}} : (('parameter_{increase_counter_param.name}_{counter_obj_param.name}@{{w}}' = 1.0) ? fluent_{counter_param.name}_{c2.name}@{{i}} : fluent_{counter_param.name}_{c1.name}@{{i}})))",
                increase_counter_param,
            ),
            # real expressions
            ...
            (
                real_counter_param(c1),
                f"fluent_{real_counter_param.name}_{c1.name}@{{i}}",
                None,
            ),
            (
                real_counter_param(c2),
                f"fluent_{real_counter_param.name}_{c2.name}@{{i}}",
                None,
            ),
            (
                real_counter_param(c3),
                f"fluent_{real_counter_param.name}_{c3.name}@{{i}}",
                None,
            ),
            (
                real_counter_param(counter_obj_param),
                f"(('parameter_{increase_counter_param.name}_{counter_obj_param.name}@{{w}}' = 2.0) ? fluent_{real_counter_param.name}_{c3.name}@{{i}} : (('parameter_{increase_counter_param.name}_{counter_obj_param.name}@{{w}}' = 1.0) ? fluent_{real_counter_param.name}_{c2.name}@{{i}} : fluent_{real_counter_param.name}_{c1.name}@{{i}}))",
                increase_counter_param,
            ),
            ...
            (
                real_counter + real_counter_param(c1),
                f"(fluent_{real_counter.name}@{{i}} + fluent_{real_counter_param.name}_{c1.name}@{{i}})",
                None,
            ),
            (
            ...
            (
                bool_counter_param(c1),
                f"fluent_{bool_counter_param.name}_{c1.name}@{{i}}",
                None,
            ),
            (
                bool_counter_param(c2),
                f"fluent_{bool_counter_param.name}_{c2.name}@{{i}}",
                None,
            ),
            (
                bool_counter_param(c3),
                f"fluent_{bool_counter_param.name}_{c3.name}@{{i}}",
                None,
            ),

...
class TestHorizon(TestCase):
    def test_horizon(self):
        ...
        for problem, min_correct_horizon in self._get_problems_with_min_horizon():
            for engine, params in engines_and_params:
                ...
                if (
                    "opt" in engine
                    and params["sat_before_opt"]
                    and min_correct_horizon >= 4
                ):
                    continue
                with OneshotPlanner(name=engine, params=params) as planner:
                    result = planner.solve(problem)
                    self.assertIsNone(
                        result.plan, f"{problem.name}, {min_correct_horizon}, {params}"
                    )
    ...
    def _basic_counter(self) -> Iterator[tuple[Problem, int]]:
        ...
        increase_counter = DurativeAction("increase counter")
        increase_counter.set_fixed_duration(1)
        increase_counter.add_increase_effect(StartTiming(Fraction(1, 2)), counter, 1)
        ...
        increase_counter.add_increase_effect(StartTiming(Fraction(2, 3)), counter, 1)
    ...
    def _move_ball(self):
        ...
        robot_loaded = base_problem.add_fluent(
            "robot_loaded", BoolType(), default_initial_value=False
        )
        ball_on_robot = base_problem.add_fluent(
            "ball_on_robot", Ball, default_initial_value=b1
        )
        robot_at = base_problem.add_fluent(
            "robot_at", Location, default_initial_value=l1
        )
        ball_at = base_problem.add_fluent(
            "ball_at", Location, default_initial_value=l1, ball=Ball
        )
    ...
    def _match_action_costs_v2(self):
        ...
        problem_filename = os.path.join(
            pddl_files_dir, "match_action_costs_v2_2_1.pddl"
        )

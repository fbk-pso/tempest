...
The `main.py` script can be used to dump the formulae. For additional info on the usage of the script you can do `python3 main.py -h`.
...
In the benchmarks folder there are 2 folders, one containing benchmarks in pddl format and the other containing python benchmarks generators.

### Generating OMT from PDDL benchmarks

To use the PDDL files it is sufficient to invoke the script with the `--pddl-domain` and `--pddl-problem` parameters, specifying the path to the domain file and to the problem file.
...
### Generating OMT using python benchmarks generators
...
The parameters `--problem-package`, `--problem-function` and `--problem-params` must be specified to generate the problem.
